...
def test(test_loader,model_main,log):
    ...
    with open("Predictions"+str(log.param.dataset)+".txt", "w") as outfile:
        np.savetxt(outfile, np.array(total_pred_1))
    with open("True"+str(log.param.dataset)+".txt", "w") as outfile:
        np.savetxt(outfile, np.array(total_true))
...
def cl_test(log):
    ...
    td,valid_data,test_data = get_dataloader(log.param.train_batch_size,log.param.eval_batch_size,log.param.dataset,w_aug=False,w_double=False,label_list=None)
    ...
    if torch.cuda.is_available():
        model_main.cuda()
    ###################################################################
    t_acc_1,t_f1_1,t_save_pred = test(td,model_main,log)
